...
const MODEL_CAPS: Record<string, number> = {"Unity XT 380": 500, "Unity XT 480": 750, "Unity XT 680": 1000, "Unity XT 880": 1500};
const RAID_SETS: Record<string, string[]> = {"RAID5": ["4+1", "8+1", "12+1"], "RAID6": ["4+2", "6+2", "8+2", "12+2", "14+2"], "RAID10": ["1+1", "2+2", "3+3", "4+4"]};
const RAID_OPTIONS: Record<string, string[]> = {"Extreme Performance": ["RAID5", "RAID6", "RAID10"], "Performance": ["RAID5", "RAID6", "RAID10"], "Capacity": ["RAID5", "RAID6", "RAID10"]};
const TIERS: Array<[string,string[]]> = [["Extreme Performance", ["400GB", "800GB", "1.6TB", "1.92TB", "3.2TB", "3.84TB", "7.68TB", "15.36TB"]], ["Performance", ["1.2TB", "1.8TB"]], ["Capacity", ["4TB", "6TB", "12TB"]]];

type Row = { disk: string; raid: string; spare: string; set: string; count: number; };
type Result = { tier: string; usableTB: number; };

function parseSet(s: string) { const [a,b] = s.split('+').map(n=>parseInt(n,10)); return {a,b,size:a+b}; }
function per32(sp: string) { return sp === "2/32" ? 2 : 1; }
function validCounts(maxN:number, setSize:number, per:number) {
  const out:number[] = [0];
  for(let n=1;n<=maxN;n++){ const sp=Math.max(per, Math.ceil(n/32)*per); const eff=n-sp; if(eff>=setSize && eff%setSize===0) out.push(n); }
  return out;
}

export default function Home() {
  const [model, setModel] = useState(Object.keys(MODEL_CAPS)[1] || "Unity XT 480");
  const [rows, setRows] = useState<Record<string, Row>>(() => {
    const init: Record<string, Row> = {}
    for (const [tier, sizes] of TIERS) {
      init[tier] = { disk: sizes[0], raid: RAID_OPTIONS[tier][0], spare:"1/32", set: RAID_SETS[RAID_OPTIONS[tier][0]][0], count: 0 }
    }
    return init;
  });
  ...
  function setRow(tier:string, patch: Partial<Row>) { setRows(prev=>({...prev, [tier]: {...prev[tier], ...patch} })) }

  function countOptions(tier:string) {
  ...
  async function onCalc() {
    ...
    try {
      const payloads = Object.entries(rows).map(([tier,r]) => fetch("/api/unity-calculator", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ disk:r.disk, raid:r.raid, set:r.set, count:r.count, sparePolicy:r.spare })
      }).then(async res => ({ tier, body: await res.json(), ok: res.ok })));
      const outs = await Promise.all(payloads);
      const ok = outs.filter(o=>o.ok).map(o=>({ tier:o.tier, usableTB:o.body.usableTB as number }));
      setResults(ok);
  ...
  return (
    <main style={ padding: 24 }>
      ...
      {TIERS.map(([tier, sizes]) => {
        const r = rows[tier];
        const raidList = RAID_OPTIONS[tier];
